...
namespace DAL_QuanLyBK
{
    public class DAL_LoaiSanPham : DBConnect
    {
        ...
        public bool addLoaiSanPham(DTO_LoaiSanPham lsp)
        {
            ...
            {
                _conn.Open();
                string SQL = string.Format("INSERT INTO LOAISANPHAM(MaLSP,TenLSP,MoTa) VALUES ('{0}',N'{1}',N'{2}')", lsp.MA_LSP, lsp.TEN_LSP, lsp.MOTA);
        ...
        public bool editLoaiSanPham(DTO_LoaiSanPham lsp)
        {
            ...
            {
                _conn.Open();
                string SQL = string.Format("UPDATE LOAISANPHAM SET TenLSP=N'{0}',MoTa=N'{1}' where MaLSP='{2}'", lsp.TEN_LSP, lsp.MOTA, lsp.MA_LSP);
                SqlCommand cmd = new SqlCommand(SQL, _conn);
                if (cmd.ExecuteNonQuery() > 0)
                {
                    return true;
